         ...
         <option value="TO" <?php if(@$estado == 'TO'){ ?> selected <?php } ?>>TO</option>
                    ...
                    <div class="col-lg-4 col-md-6">
                        <div class="checkout__order">
                          ...
                          $total_sem_frete = $total_sem_frete + $total_item;

                          $res_personalizacao = $pdo->query("SELECT * from personalizacao_itens_car where id_carrinho = '$id_carrinho'");
                          $dados_personalizacao = $res_personalizacao->fetchAll(PDO::FETCH_ASSOC);

                          if (count($dados_personalizacao) > 0) {
                           $total_sem_frete = $total_sem_frete + (15 * $quantidade);
                          }
                          if ($patrocinio == 'Sim') {
                           $total_sem_frete = $total_sem_frete + (45 * $quantidade);
                          }
                          ...
                          <?php if (count($dados_personalizacao) > 0) { ?>
                            <br>
                            <p align="right" class="text-danger"><small>Valor Personalizacao total deste item: <?php echo ($quantidade * 15) ?>.00</small></p>
                          <?php } ?>
                          <?php if ($patrocinio == 'Sim') { ?>
                            <p align="right" class="text-danger"><small>Valor Patrocinio total deste item: <?php echo ($quantidade * 45) ?>.00</small></p>
                          <?php } ?>

                          <?php }
                            $total_com_desconto = $total_sem_frete;
                            $mensagem_desconto = 'Receba um desconto de 10% ao comprar mais de 3 itens!';
                            if ($total_quantidade >= 3 && $total_quantidade < 6) {
                                $total_com_desconto = ($total_sem_frete * 9) / 10;
                                $mensagem_desconto = 'Desconto de 10% por comprar mais de 3 itens, receba 15% comprando mais de 6!';
                            } elseif ($total_quantidade >= 6 && $total_quantidade < 10) {
                                $total_com_desconto = ($total_sem_frete * 8.5) / 10;
                                $mensagem_desconto = 'Desconto de 15% por comprar mais de 6 itens, receba 20% comprando mais de 10!';
                            } elseif ($total_quantidade >= 10) {
                                $total_com_desconto = ($total_sem_frete * 8) / 10;
                                $mensagem_desconto = 'Desconto de 20% por comprar mais de 10 itens!';
                            }
                            if ($total_frete > 0) {
                                $total_com_desconto += $total_frete;
                            }
                            $total = number_format($total_com_desconto, 2, ',', '.');

                           ?>
                      </ul>
                      <div class="checkout__order__subtotal">
                      <div>
                        <small class="text-danger"><?php echo $mensagem_desconto ?></small>
                      </div>
                        Subtotal <span>R$ <?php echo $total ?></span>
                      </div>
                 ...
                 <?php } ?>
                    ...
                    <div  class="checkout__order__total">
                        Total <span id="total_final"></span>
                      </div>
