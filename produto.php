...
$id_produto = $res[0]['id'];
@$personalizavel = $res[0]['personalizavel'];
$patrocinio = $res[0]['patrocinio'];

if($modelo == ""){
    $modelo = "Nenhum";
}

$mostrar_personalizar = '';
if ($personalizavel == 'Não') {
    $mostrar_personalizar = 'd-none';
}
$mostrar_patrocinio = '';
if ($patrocinio == 'Não') {
    $mostrar_patrocinio = 'd-none';
}
...
<section class="product-details spad">
                ...
                <div class="product__details__text">
                    ...
                    <small><div id="div-mensagem-prod"></div></small>
                       ...
                       <?php if(@$tem_cor == 'Sim'){ ?>
                        ...
                        ?>
                        <div class=<?php echo($mostrar_personalizar) ?>>
                            <div class="row ml-1 mb-3">
                                <p class="mb-0">Deseja personalizar seu produto?(15,00 reais)</p>
                            </div>
                            <div class="row ml-1 mb-4">
                                <select class='form-control form-control-sm' id="personalizar" name='personalizar'>
                                    <option value="0" >NÃO</option>
                                    <option value="1">SIM</option>
                                </select>
                            </div>
                            <div class="row ml-1 mb-4 personalizacao-form col-12 pl-0 d-none">
                                <div class="col-3 pl-0">
                                    <input type="number" class="form-control" name="personalizacao_numero" placeholder="Número">
                                </div>
                                <div class="col-6">
                                    <input type="text" class="form-control" name="personalizacao_nome" placeholder="Nome">
                                </div>
                            </div>
                        </div>
                        <div class=<?php echo($mostrar_patrocinio) ?>>
                            <div class="row ml-1 mb-3">
                                <p class="mb-0">Deseja que seu produto tenha todos os patrocínios?(45,00 reais)</p>
                            </div>
                            <div class="row ml-1 mb-4">
                                <select class='form-control form-control-sm' id="patrocinio" name='patrocinio'>
                                    <option value="0" >NÃO</option>
                                    <option value="1">SIM</option>
                                </select>
                            </div>
                         ...
                         <?php
                            if($nome_frete == 'Sem Frete'){
                                echo '<b>Este produto está com Frete Gratuito!</b><p>A partir da confirmação do pagamento, o prazo médio de entrega é 10  a 30  dias úteis, dependendo da região.</p>';
                            }

                            if($nome_frete == 'Valor Fixo'){
                                 echo '<div class="product__details__text"><p>Frete Fixo de '.$valor_frete.' Reais<br>A partir da confirmação do pagamento, o prazo médio de entrega é 20 dias úteis, dependendo da região.</p>';
                            }
    ...
    <!-- Modal -->
    <div class="modal fade" id="modal-no-session" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5>
                        Você precisa fazer Login para adicionar produtos ao carrinho!! Clique <a class='text-info' href='sistema'>aqui</a> para efetuar Login!
                    </h5>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    $('#personalizar').on('change', function() {
        personalizacaoForm = $('.personalizacao-form');

        if(this.value == true) {
            personalizacaoForm.removeClass("d-none");
        } else {
            personalizacaoForm.addClass(" d-none");
            personalizacaoForm.find('input').val('');
        }
    })
...
<script type="text/javascript">
    $( document ).ready(function() {
        $('#modal-no-session').modal('hide')
        var id_usuario = "<?=$id_usuario?>";
        var nivel_usuario = "<?=$nivel_usuario?>";
        if(id_usuario == null || nivel_usuario != 'Cliente') {
            $('#modal-no-session').modal('show')
        }
    });
    $('#btn-add-car').click(function(event){
        var id_usuario = "<?=$id_usuario?>";
