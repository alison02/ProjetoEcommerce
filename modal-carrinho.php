...
<div class="modal fade" id="modal-personalizacao" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="cart-inline-title">Deseja personalizar?</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form id="form-personalizacao" method="POST">
        <div class="modal-body">
          <div class="row">
            <div class="col-md-6 mb-4" id="seletor-personalizar">
            </div>
        </div>
       </div>
      </form>
...
<!-- Modal patrocinio -->
<div class="modal fade" id="modal-patrocinio" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="cart-inline-title">Deseja os patrocínios?</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form id="form-patrocinio" method="POST">
        <div class="modal-body">
          <div class="row">
            <div class="col-md-6 mb-4" id="seletor-patrocinio">
            </div>
        </div>
       </div>
      </form>
...
<script type="text/javascript">
 function addPersonalizacao(id, id_carrinho) {
    event.preventDefault();
    $.ajax({
        url: "carrinho/personalizacao-produtos.php",
        method: "post",
        data: {id, id_carrinho},
        dataType: "text",
        success: function(mensagem){
          console.log(mensagem);
            $("#modal-personalizacao").modal("show");
            $('#seletor-personalizar').html(mensagem)
        },
    })
}
function addPatrocinio(id, id_carrinho) {
    event.preventDefault();
    $.ajax({
        url: "carrinho/patrocinio-produtos.php",
        method: "post",
        data: {id, id_carrinho},
        dataType: "text",
        success: function(mensagem){
            $("#modal-patrocinio").modal("show");
            console.log(mensagem);
            $('#seletor-patrocinio').html(mensagem)
        },
    })
}
</script>
